...
(function($, $STAN) {
  ...
  $STAN.on('window.resize fixedsize.init fixedsize.resize', function() {
  ...
  var methods = {
    ...
    fix: function() {
      ...
      $STAN.trigger('fixsize.resized', $(this), settings);
    ...
    checkSize: function(selector) {
      ...
      $(this).find(selector).each(function(index) {
        ...
        if (lastTop < thisTop + 20 && lastTop > thisTop - 20) newLine = false;
        else newLine = true;
        if (newLine && h > 0) {
